...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.listerUtilisateurs(
      jetonAcces,
      impersonation
    )
  ...
  async (requete: NextRequest) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.creerUtilisateur(
          jetonAcces,
          impersonation,
          corps
        )
