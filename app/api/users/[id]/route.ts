...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.obtenirUtilisateur(
      jetonAcces,
      impersonation,
      id
    )
...
const miseAJourUtilisateur = executerAvecGestionErreurs(
  conteneurDependances.reponseHttp,
  async (requete: NextRequest, contexte: ParametresRoute) => {
    const { id } = await contexte.params
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.mettreAJourUtilisateur(
          jetonAcces,
          impersonation,
          id,
          corps
...
export const PUT = miseAJourUtilisateur
export const PATCH = miseAJourUtilisateur
...
export const DELETE = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      serviceAuthentification: conteneurDependances.serviceAuthentification,
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.supprimerUtilisateur(
          jetonAcces,
          impersonation,
          id
        )
