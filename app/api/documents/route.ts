...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.listerDocuments(
      jetonAcces,
      impersonation
    )
  ...
  async (requete: NextRequest) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.creerDocument(
          jetonAcces,
          impersonation,
          corps
        )
