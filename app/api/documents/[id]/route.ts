...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.obtenirDocument(
      jetonAcces,
      impersonation,
      id
    )
...
const miseAJourDocument = executerAvecGestionErreurs(
  conteneurDependances.reponseHttp,
  async (requete: NextRequest, contexte: ParametresRoute) => {
    const { id } = await contexte.params
    ...
    const resultat = await conteneurDependances.controleurAdministrationAdmin.mettreAJourDocument(
      jetonAcces,
      impersonation,
      id,
      corps
...
export const PUT = miseAJourDocument
export const PATCH = miseAJourDocument
...
export const DELETE = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const jetonAcces = conteneurDependances.adaptateurRequeteSecurite.extraireJetonAcces(requete)
    const impersonation = conteneurDependances.adaptateurRequeteSecurite.lireImpersonation(requete)
    const resultat = await conteneurDependances.controleurAdministrationAdmin.supprimerDocument(
      jetonAcces,
      impersonation,
      id
    )
