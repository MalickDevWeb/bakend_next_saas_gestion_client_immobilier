...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.obtenirLocation(
      jetonAcces,
      impersonation,
      id
    )
...
const miseAJourLocation = executerAvecGestionErreurs(
  conteneurDependances.reponseHttp,
  async (requete: NextRequest, contexte: ParametresRoute) => {
    const { id } = await contexte.params
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.mettreAJourLocation(
          jetonAcces,
          impersonation,
          id,
          corps
...
export const PUT = miseAJourLocation
export const PATCH = miseAJourLocation
...
export const DELETE = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      serviceAuthentification: conteneurDependances.serviceAuthentification,
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.supprimerLocation(
          jetonAcces,
          impersonation,
          id
        )
