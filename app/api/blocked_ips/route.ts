...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.listerIpsBloquees(
      jetonAcces,
      impersonation,
      new URL(requete.url)
    )
  ...
  async (requete: NextRequest) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.bloquerIp(
          jetonAcces,
          impersonation,
          corps
        )
