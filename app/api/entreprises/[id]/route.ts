...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.obtenirEntreprise(
      jetonAcces,
      impersonation,
      id
    )
...
const miseAJourEntreprise = executerAvecGestionErreurs(
  conteneurDependances.reponseHttp,
  async (requete: NextRequest, contexte: ParametresRoute) => {
    const { id } = await contexte.params
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.mettreAJourEntreprise(
          jetonAcces,
          impersonation,
          id,
          corps
...
export const PUT = miseAJourEntreprise
export const PATCH = miseAJourEntreprise
...
export const DELETE = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      serviceAuthentification: conteneurDependances.serviceAuthentification,
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.supprimerEntreprise(
          jetonAcces,
          impersonation,
          id
        )
