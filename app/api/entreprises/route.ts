...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.listerEntreprises(
      jetonAcces,
      impersonation
    )
  ...
  async (requete: NextRequest) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.creerEntreprise(
          jetonAcces,
          impersonation,
          corps
        )
