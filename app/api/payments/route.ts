...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.listerPaiements(
      jetonAcces,
      impersonation
    )
  ...
  async (requete: NextRequest) => {
    ...
    return executerMutationIdempotenteSiDemandee({
      ...
      executerMutation: async () => {
        const resultat = await conteneurDependances.controleurAdministrationAdmin.creerPaiement(
          jetonAcces,
          impersonation,
          corps
        )
