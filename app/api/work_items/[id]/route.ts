...
export const GET = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const donnees = await conteneurDependances.controleurAdministrationAdmin.obtenirTravail(
      jetonAcces,
      impersonation,
      id
    )
...
const miseAJourTravail = executerAvecGestionErreurs(
  conteneurDependances.reponseHttp,
  async (requete: NextRequest, contexte: ParametresRoute) => {
    const { id } = await contexte.params
    ...
    const resultat = await conteneurDependances.controleurAdministrationAdmin.mettreAJourTravail(
      jetonAcces,
      impersonation,
      id,
      corps
...
export const PUT = miseAJourTravail
export const PATCH = miseAJourTravail
...
export const DELETE = executerAvecGestionErreurs(
  ...
  async (requete: NextRequest, contexte: ParametresRoute) => {
    ...
    const jetonAcces = conteneurDependances.adaptateurRequeteSecurite.extraireJetonAcces(requete)
    const impersonation = conteneurDependances.adaptateurRequeteSecurite.lireImpersonation(requete)
    const resultat = await conteneurDependances.controleurAdministrationAdmin.supprimerTravail(
      jetonAcces,
      impersonation,
      id
    )
